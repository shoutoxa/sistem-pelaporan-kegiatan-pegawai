...
describe('EditReportPage', () => {
  it('loads an editable report and sends allowed fields without replacing photos', async () => {
    const fetchMock = vi.fn(async (url, options) => {
      if (String(url).includes('/master/kategori')) return { ok: true, json: async () => [{ id: 'k1', namaKategori: 'Implementasi' }] }
      if (String(url).includes('/master/pekerjaan')) return { ok: true, json: async () => [{ id: 'p1', kategoriId: 'k1', namaPekerjaan: 'ODN' }] }
