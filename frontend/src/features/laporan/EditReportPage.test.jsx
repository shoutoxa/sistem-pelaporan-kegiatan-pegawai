...
describe('EditReportPage', () => {
  it('loads an editable report and sends allowed fields without replacing photos', async () => {
    const fetchMock = vi.fn(async (url, options) => {
      ...
      if (String(url).includes('/master/desa/d1/cluster')) return { ok: true, json: async () => [{ id: 'c1', clusterName: 'Cluster 01' }] }
      if (options?.method === 'PUT') return { ok: true, json: async () => ({ data: { id: 'r1' } }) }
      return { ok: true, json: async () => ({ data: { id: 'r1', canEdit: true, tanggalKegiatan: '2026-08-22', keterangan: 'Keterangan lama', clusterId: 'c1', pekerjaanId: 'p1', cluster: { id: 'c1', clusterName: 'Cluster 01', desa: { id: 'd1', namaDesa: 'Dewasari' } }, pekerjaan: { id: 'p1', namaPekerjaan: 'ODN' }, dokumentasi: [{ id: 'f1' }] } }) }
    })
