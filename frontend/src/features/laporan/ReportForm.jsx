...
export default function ReportForm({ user, villages, stages: stageProp }) {
  ...
  return <section className="page"><h1>Buat Laporan Kegiatan</h1><form className="report-form" onSubmit={handleSubmit}><label>PIC<input aria-label="PIC" value={user?.nama || ''} readOnly /></label><label>Tanggal Kegiatan<input type="date" value={form.tanggalKegiatan} onChange={(event) => setField('tanggalKegiatan', event.target.value)} required /></label><LocationFields value={form} onChange={(location) => setForm((current) => ({ ...current, ...location }))} desaOptions={villages} errors={{}} /><label>Tahapan<select aria-label="Tahapan" value={form.tahapanId} onChange={(event) => setForm((current) => ({ ...current, tahapanId: event.target.value, nomorPerangkat: '' }))} required><option value="">Pilih Tahapan</option>{stages.filter((stage) => stage.isActive !== false).map((stage) => <option key={stage.id} value={stage.id}>{stage.namaTahapan}</option>)}</select></label>{selectedStage?.requiresNomorPerangkat && <label>Nomor Perangkat<input aria-label="Nomor Perangkat" value={form.nomorPerangkat} onChange={(event) => setField('nomorPerangkat', event.target.value)} required /></label>}<label>Keterangan<textarea aria-label="Keterangan" value={form.keterangan} onChange={(event) => setField('keterangan', event.target.value)} required /></label><FilePicker files={files} onChange={setFiles} />{error && <p role="alert">{error}</p>}<button type="submit" disabled={submitting}>{submitting ? 'Mengirim...' : 'Kirim Laporan'}</button></form></section>
}
