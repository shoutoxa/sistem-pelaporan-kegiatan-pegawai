...
function sendError(error, response) {
  const statuses = { VALIDATION: 400, FILE_VALIDATION: 400, FILE_LIMIT: 413, DATE_VALIDATION: 422, REFERENCE_INVALID: 422, NOT_FOUND: 404, STORAGE_ERROR: 500, DATABASE_ERROR: 500 }
...
export async function createProductionReportRouter({ authService } = {}) {
  ...
  return createReportRouter({ reportService, requireAuth: requireAuth({ authService: sessionService }), requirePegawai: [requireAuth({ authService: sessionService }), requireRole('PEGAWAI')] })
}
